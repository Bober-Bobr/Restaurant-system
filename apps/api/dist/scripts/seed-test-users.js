...
async function upsertUser(username, role, restaurantId) {
...
async function main() {
    ...
    const score = (r) => r._count.events + r._count.halls + r._count.menuItems + r._count.tableCategories;
    console.log('\nFound restaurants:');
    allRestaurants.forEach((r) => console.log(`  ${r.id}  "${r.name}"  score=${score(r)}`));
    ...
    const madinCandidates = allRestaurants.filter((r) => r.name.toLowerCase().includes('madinabek'));
    ...
    if (!primarySource)
        throw new Error('No restaurants found in database. Nothing to assign.');
    ...
    const emptyDuplicates = madinCandidates.filter((r) => r.id !== primarySource.id && score(r) === 0);
    ...
    const restCandidates = allRestaurants.filter((r) => r.id !== madinabekRestaurant.id && r.name.toLowerCase().includes('rest'));
    const restFallback = allRestaurants.filter((r) => r.id !== madinabekRestaurant.id);
