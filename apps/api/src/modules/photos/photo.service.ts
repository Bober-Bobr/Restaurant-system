...
export class PhotoService {
  ...
  private restaurantSlug(restaurantId: string | null): string {
    return restaurantId ?? 'shared';
  }

  private getBaseDir(restaurantId: string | null): string {
  ...
  async uploadPhotos(category: PhotoCategory, files: Express.Multer.File[], restaurantId: string | null, dishCategory?: string): Promise<string[]> {
  ...
  async listPhotos(category: PhotoCategory, restaurantId: string | null, dishCategory?: string): Promise<string[]> {
    const baseDir = this.getBaseDir(restaurantId);
    const slug = this.restaurantSlug(restaurantId);
    ...
    if (category === 'menu' && dishCategory) {
      return this.listFilesInDir(
        ...
        `/uploads/${slug}/${category}/${dishCategory}`
      );
    }
  ...
  async deletePhoto(category: PhotoCategory, filename: string, restaurantId: string | null, dishCategory?: string): Promise<void> {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === 'ENOENT') throw createHttpError(404, 'Photo not found');
