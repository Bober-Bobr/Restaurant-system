...
describe('allocating an event number', () => {
  it('counts from 1 within each restaurant, independently', async () => {
    ...
    expect((await repo.create('r-a', 'BANQUET', PAYLOAD)).eventNumber).toBe(1);
    expect((await repo.create('r-a', 'BANQUET', PAYLOAD)).eventNumber).toBe(2);
    expect((await repo.create('r-b', 'BANQUET', PAYLOAD)).eventNumber).toBe(1);
    expect((await repo.create('r-b', 'BANQUET', PAYLOAD)).eventNumber).toBe(2);
  });
  ...
  it('a restaurant well behind another still books', async () => {
    ...
    const created = await new EventRepository().create('r-b', 'BANQUET', PAYLOAD);
  ...
  it('gives up rather than spinning forever', async () => {
    ...
    await expect(new EventRepository().create('r-a', 'BANQUET', PAYLOAD)).rejects.toMatchObject({ code: 'P2002' });
  });
  ...
  it('does not retry an unrelated failure', async () => {
    ...
    await expect(new EventRepository().create('r-a', 'BANQUET', PAYLOAD)).rejects.toThrow('connection lost');
...
describe('listing a restaurant\'s events', () => {
  ...
  it('returns every event when no page is asked for', async () => {
    seed(57);
    expect(await new EventRepository().list('r-a', 'BANQUET')).toHaveLength(57);
  });

  it('includes the newest booking, which is what went missing', async () => {
    seed(20);
    const created = await new EventRepository().create('r-a', 'BANQUET', PAYLOAD);
    const listed = await new EventRepository().list('r-a', 'BANQUET');
    expect(listed.map((e) => e.eventNumber)).toContain(created.eventNumber);
  });

  it('still pages when a page is asked for', async () => {
    seed(57);
    const page = await new EventRepository().list('r-a', 'BANQUET', { skip: 20, take: 20 });
