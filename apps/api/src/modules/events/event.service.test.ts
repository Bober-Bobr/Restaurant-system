...
describe('an event is addressed by its number, not its database id', () => {
  it('exposes eventNumber as `id` and hides the cuid', async () => {
    ...
    const event = await service.getEventDetails('r1', 'BANQUET', 42);
  ...
  it('maps every event in a list the same way', async () => {
    const { service } = makeService();
    const events = await service.listEvents('r1', 'BANQUET', { skip: 0, take: 10 });
  ...
  it('always resolves the number within the caller\'s restaurant', async () => {
    ...
    await service.getEventDetails('r1', 'BANQUET', 42);
    expect(repo.getByNumber).toHaveBeenCalledWith('r1', 'BANQUET', 42);
  });

  const SCOPED: { name: string; run: (s: InstanceType<typeof EventService>) => Promise<unknown> }[] = [
    { name: 'reading', run: (s) => s.getEventDetails('r1', 'BANQUET', 42) },
    { name: 'editing', run: (s) => s.updateEvent('r1', 'BANQUET', 42, {}) },
    { name: 'deleting', run: (s) => s.deleteEvent('r1', 'BANQUET', 42) },
    { name: 'rescheduling', run: (s) => s.rescheduleEvent('r1', 'BANQUET', 42, new Date()) },
    { name: 'adding a payment', run: (s) => s.addPayment('r1', 'BANQUET', 42, 1000) },
    { name: 'removing a payment', run: (s) => s.removePayment('r1', 'BANQUET', 42, 'pay1') },
  ];
  ...
  it('does not delete anything when the event is not found', async () => {
    ...
    await statusOf(() => service.deleteEvent('r1', 'BANQUET', 42));
...
describe('the expense-ledger mirror', () => {
  it('runs when an event is created', async () => {
    const { service } = makeService();
    await service.createEvent('r1', 'BANQUET', { customerName: 'Aziz' } as never);
  ...
  it('runs again when the details change', async () => {
    ...
    const { service } = makeService();
    await service.updateEvent('r1', 'BANQUET', 42, { guestCount: 200 } as never);
    expect(ledgerSync).toHaveBeenCalledTimes(1);
  });

  it('runs when an event is moved to another date', async () => {
    const { service } = makeService();
    await service.rescheduleEvent('r1', 'BANQUET', 42, new Date('2026-06-01T00:00:00.000Z'));
    expect(ledgerSync).toHaveBeenCalledTimes(1);
  });
});
...
describe('rescheduling', () => {
  it('records where the event started out', async () => {
    ...
    const data = repo.updateByNumber.mock.calls[0][3] as { originalEventDate: Date; eventDate: Date };
  ...
  it('KEEPS the first original date across repeated moves', async () => {
    ...
    await service.rescheduleEvent('r1', 'BANQUET', 42, new Date('2026-07-01T00:00:00.000Z'));
    expect((repo.updateByNumber.mock.calls[0][3] as { originalEventDate: Date }).originalEventDate).toEqual(firstDate);
  });
});
...
describe('editing an event', () => {
  it('leaves a hall alone when the payload does not mention it', async () => {
    ...
    const { service, repo } = makeService();
    await service.updateEvent('r1', 'BANQUET', 42, { guestCount: 200 } as never);
    expect(repo.updateByNumber.mock.calls[0][3]).not.toHaveProperty('hallId');
  });

  it('clears a hall when it is explicitly emptied', async () => {
    const { service, repo } = makeService();
    await service.updateEvent('r1', 'BANQUET', 42, { hallId: '' } as never);
    expect((repo.updateByNumber.mock.calls[0][3] as { hallId: null }).hallId).toBeNull();
  });

  it('treats the table categories and the debt deadline the same way', async () => {
    const { service, repo } = makeService();
    await service.updateEvent('r1', 'BANQUET', 42, {
      tableCategoryId: '', childrenTableCategoryId: '', debtDeadline: null,
    } as never);
    const data = repo.updateByNumber.mock.calls[0][3] as Record<string, unknown>;
  ...
  it('passes the menu selection through untouched', async () => {
    ...
    await service.updateEvent('r1', 'BANQUET', 42, { menuConfig } as never);
    expect((repo.updateByNumber.mock.calls[0][3] as { menuConfig: unknown }).menuConfig).toEqual(menuConfig);
  });
});
...
describe('a payment can never exceed the balance', () => {
  ...
  it('takes a payment for exactly the outstanding balance', () => {
    ...
    return expect(service.addPayment('r1', 'BANQUET', 42, 1_000_000)).resolves.toBeDefined();
  });

  it('takes a smaller instalment', () => {
    const { service } = serviceFor(invoiced());
    return expect(service.addPayment('r1', 'BANQUET', 42, 400_000)).resolves.toBeDefined();
  });

  it('refuses one so'.concat("'", 'm over'), async () => {
    const { service, repo } = serviceFor(invoiced());
    await expect(service.addPayment('r1', 'BANQUET', 42, 1_000_100)).rejects.toMatchObject({ status: 400 });
  ...
  it('counts the deposit and earlier instalments towards the cap', () => {
    ...
    return Promise.all([
      expect(service.addPayment('r1', 'BANQUET', 42, 500_000)).resolves.toBeDefined(),
      expect(serviceFor(event).service.addPayment('r1', 'BANQUET', 42, 500_001)).rejects.toMatchObject({ status: 400 }),
    ]);
  });
  ...
  it('counts priced dish selections into the total', () => {
    ...
    const { service } = serviceFor(event);
    return expect(service.addPayment('r1', 'BANQUET', 42, 1_100_000)).resolves.toBeDefined();
  });

  it('refuses anything at all once the invoice is settled', async () => {
    const event = invoiced({ depositCents: 1_000_000 });
    await expect(serviceFor(event).service.addPayment('r1', 'BANQUET', 42, 1)).rejects.toMatchObject({ status: 400 });
  });

  it('still 404s for an event that does not exist, before any of this', () => {
    const { service } = serviceFor(null);
    return expect(service.addPayment('r1', 'BANQUET', 42, 1)).rejects.toMatchObject({ status: 404 });
  });
});
