...
// Unanswered-request counter shown beside the Notifications nav item.
const NavBadge = ({ count }: { count: number }) => (
  <span style={{
    minWidth: 18, height: 18, padding: '0 5px', borderRadius: 999,
    background: '#db2777', color: '#fff', fontSize: 11, fontWeight: 800, lineHeight: 1,
...
export const PerformerLayout = () => {
  ...
  // Pending booking requests drive the badge on the Notifications item.
  const pendingQuery = useQuery({
    queryKey: ['pf-bookings-pending'],
    queryFn: () => performerService.pendingCount(),
  ...
  const navItems: { to: string; label: string; badge?: number }[] = [
    ...
    { to: '/calendar', label: t('pf_calendar') },
    { to: '/bookings', label: t('pf_bookings'), badge: pending },
          ...
          >
            {navItems.map((item) => (
              <Link key={item.to} to={item.to} onClick={() => setMobileNavOpen(false)}
                style={{ ...linkStyle(isActive(item.to)), padding: '11px 16px', borderRadius: 10, fontSize: 14, display: 'flex', alignItems: 'center', gap: 8 }}>
                {item.label}
                {!!item.badge && <NavBadge count={item.badge} />}
              </Link>
            ))}
