...
// Upload a video from the device (Design+ overlay/cover videos). Mirrors
// AudioUploadField; stores under the shared `invitation` uploads folder.
export const VideoUploadField = ({ value, onChange, restaurantId, label }: Props) => {
    ...
    <div style={{ display: 'grid', gap: 6 }}>
      ...
      {src ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 10, borderRadius: 12, background: 'rgba(15,23,42,0.5)', border: '1px solid rgba(255,255,255,0.08)' }}>
          <video src={src} controls muted playsInline style={{ flex: 1, minWidth: 0, maxHeight: 120, borderRadius: 8, background: '#000' }} />
